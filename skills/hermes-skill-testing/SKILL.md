...
| `judge_backend` | 否 | 语义断言使用的 judge backend，默认 `hermes`；codex/claude_code 测试时建议显式设置 |
...
- Codex / Claude Code backend 通过 `command_template` 驱动，默认模板已内置必要的非交互标志
...
| Session 捕获 | ✅ sessions/ 目录 | ❌ 无 | ❌ 无 |
| 语义 judge | ✅ 原生支持 | ❌ 需设 judge_backend=hermes | ❌ 需设 judge_backend=hermes |
| 多轮 resume | ✅ --resume session_id | ❌ 每轮独立进程 | ❌ 每轮独立进程 |
| 工具调用无拦截 | ✅ HERMES_ACCEPT_HOOKS=1 | ✅ --dangerously-bypass-approvals-and-sandbox | ✅ --dangerously-skip-permissions |

### Codex 特有约束

**非交互模式**：默认模板已包含 `--dangerously-bypass-approvals-and-sandbox`，跳过所有确认提示。如需限制沙箱权限，在 spec 里覆盖 `command_template` 并去掉该标志。

**无 session 捕获**：Codex 每轮是独立进程，`transcript` 字段始终为空。多轮测试中每轮状态不延续，需在 prompt 里重新提供上下文。

**语义断言**：Codex backend 不实现 `run_semantic_judge`，调用时返回失败而非崩溃。设置 `judge_backend: "hermes"` 可将语义断言路由到 Hermes 执行（需 Hermes 可用）。

**模型选择**：Codex 用 `-m <model>` 而非 `--model`；在 spec 里设置 `model` 字段时，需通过 `command_template` 手动传入，或使用 `-c model="..."` 覆盖配置。
...
**语义断言**：同 Codex，设置 `judge_backend: "hermes"` 可路由到 Hermes 执行。
