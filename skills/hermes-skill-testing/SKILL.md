...
| 字段 | 必填 | 含义 |
|------|------|------|
| `backend` | 否 | `hermes`、`codex` 或 `claude_code`；缺省为 `claude_code` |
| `skill` | 是 | 目标 skill 名称 |
| `timeout` | 否 | 每轮超时秒数 |
...
**斜杠命令路由**：`claude -p "/skill-name ..."` 会被 Claude Code CLI 的路由层拦截，在 LLM 看到 prompt 之前就返回 `"Unknown skill: <name>"`。因此 claude_code 模式的测试 prompt 必须使用自然语言，不能用 `/skill-name` 前缀。
...
**语义断言**：claude_code backend 未实现 `run_semantic_judge`。避免在 claude_code spec 中使用 `semantic`/`not_semantic` 断言，或设置 `judge_backend: "hermes"` 并确保 Hermes 可用。
