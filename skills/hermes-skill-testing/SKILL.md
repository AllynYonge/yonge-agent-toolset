...
1. 分析目标 skill 的关键约束、复杂度和风险面。
2. 输出准备测试的 case 清单，等待用户确认或修正测试范围。
3. 在确认后的 case 清单基础上编写 JSON 行为规范（spec）。
4. 运行测试框架 `references/dynamic_skill_behavior_harness.py`。
5. 检查失败断言和实际模型输出。
6. 最小化修补目标 skill。
7. 重复直到所有断言通过。
...
遵循 TDD 风格；但 RED 之前必须完成 case 预确认：

0. **Case 预确认** — 输出准备测试的 case 清单，等待用户确认或修正。
1. **RED** — 在确认后的 case 清单基础上编写针对行为缺口的规范，预期会失败。
...
**case 数量由复杂度决定**。先按目标 skill 的关键约束、交互步骤、工具/副作用风险等评估复杂度，再确定 case 数量：简单 skill 通常覆盖 2-3 个核心 case；中等复杂度 skill 通常覆盖 4-6 个 case；复杂或高风险 skill 通常覆盖 7-10 个 case。数量不是硬指标，必须以可验证风险为依据；不要为凑数量添加冗余 case，也不要因 case 较少而遗漏关键约束。

### Case 预确认闸门

在编写或输出 JSON spec 前，必须先给用户输出准备测试的 case 清单，并停止等待用户确认或修正。不得在用户确认前创建、保存或展示完整 spec。

清单必须包含：
- 复杂度判断：简单 / 中等 / 复杂或高风险，以及判断依据
- 计划 case 数量：说明为什么这个数量足以覆盖关键风险
- 每个 case 的 `id`、目标约束、用户输入概要、断言重点和风险维度
- 明确询问用户这些 case 是否满足测试需求；用户确认后，才能继续编写 spec、运行 RED 和后续迭代
...
- [ ] 已按目标 skill 复杂度确定 case 数量
- [ ] 已在输出 JSON spec 前给用户展示 case 清单并获得确认
