...
class AgentBackend:
    ...
    def find_skill_dir(self, skill_name: str) -> tuple[Path, Path]:
        ...
        # Direct check for symlinked skill directories (rglob doesn't follow symlinks by default in Python 3.13+)
        direct = src_root / skill_name / "SKILL.md"
        if direct.exists():
            return direct.parent, direct.parent.relative_to(src_root)
